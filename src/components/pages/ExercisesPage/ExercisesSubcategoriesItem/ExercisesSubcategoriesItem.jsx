...
const ExercisesSubcategoriesItem = () => {
    ...
    useEffect(() => {
        if (data) {
            switch (categories) {
                case 'body-parts': {
                    const bodyParts = data.filter(
                        (element) => element.filter === 'Body parts',
                    );
                    upperCaseName(bodyParts);
                    break;
                }
                case 'muscles': {
                    const muscles = data.filter(
                        (element) => element.filter === 'Muscles',
                    );
                ...
                case 'equipment': {
                    const equipment = data.filter(
                        (element) => element.filter === 'Equipment',
                    );
            ...
            {!isLoading && (
                <Slider {...settings}>
                    {exercisesData.map((item) => (
                        <Link
                            to={`/exercises/${categories}/${item.name.toLowerCase()}`}
                            key={item._id}
                        >
